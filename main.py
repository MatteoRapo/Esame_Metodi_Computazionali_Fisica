...
pacchetto_onda=generatore_pacchetti_onda.pacchetto_onda(int(N),xx,dispersione=dispersione_in,distribuzione=distribuzione_in)
''' faccio il plot del pacchetto d'onda, fft produce il grafico di un periodo del pacchetto d'onda'''
plt.plot(xx[:len(pacchetto_onda.iniziale)],pacchetto_onda.iniziale[:len(pacchetto_onda.iniziale)].real,color='blue',label='Parte reale')
plt.plot(xx[:len(pacchetto_onda.iniziale)],pacchetto_onda.iniziale[:len(pacchetto_onda.iniziale)].imag,'--',label='parte immaginaria',color='orange')
plt.xlabel('x [m]')
...
def animazione(t):
    ...
    ampiezza_quadra = np.abs(pacchetto_onda.evoluzione_temporale(xx , t/4))**2 #il tempo viene diviso per 4 perchè                                                                                     risultava troppo veloce il dt unitario 
...
def init():
    ...
    ax.set_ylim(min(np.abs(pacchetto_onda.evoluzione_temporale(xx,0)**2)), max(np.abs(pacchetto_onda.evoluzione_temporale(xx,0)**2)))
    ax.set_xlabel('x [m]')
    ax.set_ylabel('potenza [u.a.]')
    return linea1,


time_text=ax.text(max(xx)*(3/4),max(np.abs(pacchetto_onda.evoluzione_temporale(xx,0)**2))*(3/4),'',bbox=dict(facecolor='white', edgecolor='black'))
